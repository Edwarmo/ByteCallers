...
export const PerformanceChart: React.FC<PerformanceChartProps> = ({ 
  ...
  // Tomar los 3 servicios principales con mejor tasa de resolución
  const topPerformers = serviceStats
    .sort((a, b) => b.resolutionRate - a.resolutionRate)
    .slice(0, 3);

  // Si no hay datos, usar valores por defecto
  const chartData = topPerformers.length > 0
    ? {
        x: topPerformers.map(s => s.serviceLabel),
        y: topPerformers.map(s => s.resolutionRate / 100), // Convertir porcentaje a decimal (0-1)
      }
    : {
