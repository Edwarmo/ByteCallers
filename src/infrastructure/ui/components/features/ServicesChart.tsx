...
export const ServicesChart: React.FC<ServicesChartProps> = ({ 
  ...
  // Tomar hasta 5 servicios, ordenados por casos (mayor a menor)
  const topServices = serviceStats
    .sort((a, b) => b.cases - a.cases)
    .slice(0, 5);

  // Si no hay datos, usar valores por defecto
  const chartData = topServices.length > 0
    ? {
        labels: topServices.map(s => s.serviceLabel),
        values: topServices.map(s => Math.round(s.resolutionRate)),
      }
    : {
  ...
  const barWidth = 50;
  const numBars = Math.max(1, chartData.labels.length); // Asegurar al menos 1 barra
  const spacing = (chartWidth - barWidth * numBars) / (numBars + 1);
